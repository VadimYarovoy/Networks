  ...
  *Входные данные*:
  #align(center)[
    #tablex(
      ...
      columns: 8,
      auto-vlines: false,
      [Место], [$f_c$], [$h_(t e)$], [$h_(r e)$], [$d$], [$C_m$], [$P_t$], [$P_("crit")$],
  ...
  *Входные данные*:
  ...
  #align(center)[
    #tablex(
      ...
      columns: 9,
      auto-vlines: false,
      [Место], [$f_c$], [$h$], [$W$], [$d$], [$h_(B S)$], [$h_(U T)$], [$P_t$],  [$P_("crit")$],
...
- *4G* - превосходит типовое значение, значения скорости отличается между эксперементами, сеть является самой используемой в текущий момент времени, поэтому ресурсы сети напрямую зависят от кол-ва подключенных пользователей и качества сигнала и потеренных пакетов, более того можно предположить что происходит переключение категорий LTE cat, что также влияет на скорость передачи данных
